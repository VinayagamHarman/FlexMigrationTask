...
goog.provide('XML');
/* Royale Dependency List: Namespace,QName,XMLList,org.apache.royale.debugging.assertType,org.apache.royale.language.toAttributeName,org.apache.royale.utils.Language,XML*/
...
XML = function(xml) {
  ...
  if (xml != null) {
    var /** @type {string} */ xmlStr = XML.ignoreWhitespace ? XML.trimXMLWhitespace("" + xml) : "" + xml;
    if (xmlStr.indexOf("<") == -1) {
      this._value = xmlStr;
    } else {
  ...
  if (!XML._class_initialized) {
    Object.defineProperty(XML.prototype, "0", {"get":function() {
      return org.apache.royale.utils.Language.as(this, XML);
    }, "set":function() {
...
XML.getQName = function(localName, prefix, uri, isAttribute) {
  ...
  uri = uri || '';
  var /** @type {string} */ key = localName + ":" + prefixKey + ":" + uri + ":" + isAttribute;
...
XML.iterateElement = function(node, xml) {
  ...
  for (i = 0; i < len; i++) {
    var /** @type {Node} */ nativeNode = childNodes[i];
...
XML.fromNode = function(node) {
  ...
  switch (node.nodeType) {
    ...
    case 3:
      ...
      xml = new XML();
      xml.setValue(org.apache.royale.utils.Language.string(data));
      break;
    case 4:
      xml = new XML();
      data = "<![CDATA[" + data + "]]>";
...
XML.prototype.addNamespace = function(ns) {
  ...
  for (i = 0; i < len; i++) {
    ...
    if (att.name().prefix == ns.prefix) {
      att._name = XML.getQName(att._name.localName, null, att._name.uri, true);
    }
  }
  return this;
};
...
var wrapper = new XML();
  ...
  this.XML_appendChildInternal(child);
  return this;
};
 ...
 * @param {*} child
 */
XML.prototype.XML_appendChildInternal = function(child) {
...
XML.prototype.equals = function(xml) {
  ...
  if (!org.apache.royale.utils.Language.is(xml, XML))
    return false;
  if (xml.getNodeRef() != this.getNodeRef())
    return false;
...
XML.prototype.processingInstructions = function(name) {
  ...
  var /** @type {number} */ len = this.childrenLength();
  for (i = 0; i < len; i++) {
    if (org.apache.royale.utils.Language.as(this._children[i], XML).getNodeRef() == XML.PROCESSING_INSTRUCTION)
      list.append(this._children[i]);
  }
  list.targetObject = this;
  return list;
...
XML.prototype.setNamespace = function(ns) {
  ...
  if (kind == XML.ATTRIBUTE) {
    ...
    this.XML__parent.addNamespace(ns2);
  }
...
XML.prototype.toString = function() {
  ...
  if (kind == XML.ATTRIBUTE)
    return this._value;
  if (kind == XML.TEXT)
    return org.apache.royale.utils.Language.string(this._value && this._value.indexOf('<![CDATA[') == 0 ? this._value.substring(9, this._value.length - 3) : this._value);
  if (kind == XML.COMMENT)
    return "";
  ...
  if (this.hasSimpleContent()) {
    ...
    for (i = 0; i < len; i++) {
      ...
      if (child.child('childKind') == XML.COMMENT || child.child('childKind') == XML.PROCESSING_INSTRUCTION)
        continue;
      s = s + child.toString();
    }
    return s;
 ...
 * @return {string}
 */
XML.prototype.XML__toXMLString = function(indentLevel, ancestors) {
...
var nodeType = this.getNodeRef();
  if (nodeType == XML.TEXT) {
    ...
    if (this._value.indexOf('<![CDATA[') == 0)
      return this._value;
  ...
  for (i = 0; i < len; i++) {
    ...
    strArr.push('="');
    strArr.push(XML.escapeAttributeValue(org.apache.royale.utils.Language.string(this._attributes[i].getValue())));
    strArr.push('"');
  }
  for (i = 0; i < declarations.length; i++) {
    var /** @type {string} */ decVal = XML.escapeAttributeValue(org.apache.royale.utils.Language.string(declarations[i].uri));
    if (decVal) {
      strArr.push(" xmlns");
  ...
  for (i = 0; i < len; i++) {
    if (XML.prettyPrinting && indentChildren)
      strArr.push("\n");
    strArr.push(XML.conversion(this._children[i]).XML__toXMLString(nextIndentLevel, declarations.concat(ancestors)));
  }
  if (XML.prettyPrinting && indentChildren) {
    strArr.push("\n");
    if (indentLevel > 0)
